...
function collect_handle() {
	$(".collection").click(function () {
		if ($(this).children("span").hasClass("fa-heart-o")) {
			...
			//第一行
			obj.search.push({ "type": "input", "value": { "caseNumber": $("#caseNumber").val() }, "isData":  _is_null($("#caseNumber") ) });
			obj.search.push({ "type": "input", "value": { "caseName": $("#caseName").val() }, "isData":  _is_null($("#caseName")) });
			obj.search.push({ "type": "select", "value": { "caseType": $("#caseType").val() }, "isData":  _is_null($("#caseType")) });
			obj.search.push({ "type": "select", "value": { "systemId": $("#systemId").val() }, "isData":  _is_null($("#systemId")) });
			//第二行
			obj.search.push({ "type": "select", "value": { "archiveStatus": $("#archiveStatus").val() }, "isData":  _is_null($("#archiveStatus")) });
			obj.search.push({ "type": "input", "value": { "userName": $("#userName").val() }, "isData":  _is_null($("#userName")) });
...
function initTable() {
	...
	$("#caseList").jqGrid({
		...
		colNames: ['id', '案例编号', '案例名称', '案例类型', '所属系统', '归档状态', '创建人', ' 操作'],
		colModel: [{ name: 'id', index: 'id', hidden: true },
		{
			...
			formatter: function (value, grid, row, index) {
				...
				if (row.archiveStatus == 1 && testCaseArchive == true) {
					var b = '<a  href="javascript:void(0);" class="edit-opt" onclick="archive(' + rows + ')">归档</a>';
					opt.push(b);
				}
				var c = '<a  href="javascript:void(0);" class="edit-opt" onclick="deleteTestCase(' + rows + ')">删除</a>';
...
function archive(rows) {
	findArchivedId();
	var ids = [0];
	if (aids.indexOf(Number(rows.id)) == -1) {
		ids.push(rows.id);
	};
	layer.alert('确认归档此案例？', {
		icon: 0,
...
function exportTestCase_btn() {
	...
	window.location.href = "/testManage/testCase/exportExcel?caseNumber=" + caseNumber2 + "&caseName=" + caseName2 + "&caseType=" + caseType2 +
		"&systemIds=" + systemIds2 + "&archiveStatus=" + archiveStatus2 + "&uIds=" + uIds2 + "&filters=" + ff;
}
...
function upload() {
	...
	$("#loading").css('display', 'block');
	$.ajax({
		url: "/testManage/testCase/importExcel",
		type: "POST",
		data: formData,
		/**
...
function addCheckBox() {
	...
	str = '<div class="onesCol"><input type="checkbox" value="caseNumber" onclick="showHideCol( this )" /><span>案例编号</span></div>' +
		...
		'<div class="onesCol"><input type="checkbox" value="caseType" onclick="showHideCol( this )" /><span>案例类型</span></div>' +
