...
function collect_handle() {
	$(".collection").click(function () {
		if ($(this).children("span").hasClass("fa-heart-o")) {
			...
			obj.search.push({ "type": "select", "value": { "caseType": $("#archivedCaseType").val() }, "isData": _is_null($("#archivedCaseType"))  });
			obj.search.push({ "type": "select", "value": { "systemIds": $("#systemIds").val() }, "isData": _is_null($("#systemIds"))  });
			//第二行
			...
			for (var i = 0; i < $('#colGroup2 .onesCol').length; i++) {
				obj.table.push({ "value": $('#colGroup2 .onesCol input').eq(i).attr('value'), "checked": $('#colGroup2 .onesCol input').eq(i).prop('checked') })
			}
...
function initTable() {
	...
	$("#archivedCaseList").jqGrid({
		...
		colNames: ['id', '案例编号', '案例名称', '案例类型', '所属系统', '创建人', ' 操作'],
		colModel: [{ name: 'id', index: 'id', hidden: true },
		{
			...
			formatter: function (value, grid, row, index) {
				...
				var c = '<a  href="javascript:void(0);" class="edit-opt" onclick="removeArchivedCase(' + rows + ')">案例移除</a>';
				var opt = [];
...
function exportTestCase_btn() {
	window.location.href = "/testManage/testArchivedCase/exportExcel?caseNumber=" + caseNumber2 + "&caseName=" + caseName2 + "&caseType=" + caseType2 +
		"&systemIds=" + systemIds2 + "&uIds=" + uIds2 + "&filters=" + ff;
...
function upload() {
	...
	$("#loading").css('display', 'block');
	$.ajax({
		url: "/testManage/testArchivedCase/importExcel",
		type: "POST",
		data: formData,
