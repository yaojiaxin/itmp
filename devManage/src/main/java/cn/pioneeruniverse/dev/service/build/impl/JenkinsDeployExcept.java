...
public class JenkinsDeployExcept extends JenkinsBase {
	...
	private String jenkinsUploadIp;
	@Value("${jenkins.upload.username}")
	private String jenkinsUploadUsername;
	@Value("${jenkins.upload.password}")
	private String jenkinsUploadPassword;
	...
	public void assembleExpect(Map<String, Object> paramMap, TblSystemInfo tblSystemInfo, TblToolInfo jenkinsTool, TblSystemModule tblSystemModule, 
		...
		String serverIpStr = "";
		for (int serverIndex = 0; serverIndex < validServerInfoList.size(); serverIndex++) {
		...
		StringBuilder expectScript = getServerOrderScript(tblServerInfoList);
...
//		StringBuilder detailSb = new StringBuilder();
	...
	private String getConfigPackageScript(Map<String, Object> paramMap, TblSystemInfo tblSystemInfo, TblSystemModule tblSystemModule, 
		...
		/**package下的工程包移到运行目录。*/
		msgSb.append("\ncd ").append(remoteAddr).append("\\r\n");
		msgSb.append("unzip -oq ").append(zipPackage).append("\\r\n");
		msgSb.append("rm -f ").append(zipPackage).append("\\r\n");
		msgSb.append("mv -f ").append("package/").append(mvnPackage).append(" .").append("\\r\n");
		/**configuration根据环境编码将对应环境编码下的配置文件移到运行目录。*/
		msgSb.append("mv -f configuration/").append(envName).append("/*").append(" . > /dev/null 2>&1 ").append("\\r\n");
		msgSb.append("sleep 2").append("\\r\n");
		msgSb.append("rm -rf configuration sql package").append("\\r\n");
//		msgSb.append("if [ -d \"").append(configurationDir).append("\" ];then ")
//		.append("mv -f configuration/").append(envName).append("/*").append(" . ").append("; else echo \"")
//		.append(configurationDir).append("环境不存在，配置文件不变更。\"; fi").append("\n");
