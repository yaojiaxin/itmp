...
public class SystemInfoServiceImpl
	 ...
	 */
	@Override
	public List<TaskProjectVO> getProjectListByProjectName(String projectName,String projectCode, Long uid, List<String> roleCodes,int pageNumber, int pageSize) {
...
//			systemModule.setGroupId(systemInfo.getGroupId());
	...
	public void deleteSystem(HttpServletRequest request, TblSystemInfo tblSystemInfo, List<TblSystemModule> moduleList) {
		...
		if (moduleList != null) {
			for (TblSystemModule systemModule : moduleList) {
				deleteModule(request, systemModule);
			}
		}
		TblSystemScm systemScm = new TblSystemScm();
		systemScm.setStatus(2);
		systemScm.setLastUpdateBy(userId);
		systemScm.setLastUpdateDate(stamp);
		
		TblSystemScm scmWhere = new TblSystemScm();
		scmWhere.setSystemId(tblSystemInfo.getId());
		systemScmMapper.update(systemScm, new EntityWrapper<TblSystemScm>(scmWhere));
		
		tblSystemInfo.setStatus(2);
		tblSystemInfo.setLastUpdateBy(userId);
		tblSystemInfo.setLastUpdateDate(stamp);
		updateByPrimaryKeySelective(tblSystemInfo);
	}
	...
	public String judgeJobNames(String systemJenkins,long systemId) {
		...
		if (systemJenkins != null && !"".equals(systemJenkins)) {
			...
			for (int i = 0; i < jsonArray.size(); i++) {
				...
				paramMap.put("job_type",jobType);
				paramMap.put("system_id",systemId);
				paramMap.put("status",1);
				int size=tblSystemJenkinsMapper.selectByMap(paramMap).size();
				if(size>0) {
					existJobName.append(jobName+",");
				}
				}
...
//					}
					...
					systemDeployMapper.updateByPrimaryKeySelective(systemDeploy);
				}else {   //新增部署配置
...
//		    Object env = dictMap.get(s);
	...
	public void updateScm(List<TblSystemScm> scmList,HttpServletRequest request) {
		for (TblSystemScm sScm:scmList){
			TblSystemScmRepository scmRepository=scmRepositoryMapper.
					findScmRepositoryById(sScm.getSystemRepositoryId());
			TblSystemScm systemScm = systemScmMapper.
					findScmDoesItExist(sScm.getSystemId(),sScm.getEnvironmentType());
			if(systemScm==null){
				if(sScm.getId()!=null){
