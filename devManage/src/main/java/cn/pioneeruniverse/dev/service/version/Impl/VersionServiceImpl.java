...
public class VersionServiceImpl implements VersionService {
    ...
    public JqGridPage<TblUserInfoDTO> getSvnUserAuth(JqGridPage<TblUserInfoDTO> jqGridPage, String ip, String repositoryName, String path) throws Exception {
        ...
        if (svnUserAuth != null && !svnUserAuth.isEmpty()) {
            for (Map.Entry<String, String> entry : svnUserAuth.entrySet()) {
                TblUserInfoDTO user = devManageToSystemInterface.getCodeBaseUserDetailByUserScmAccount(entry.getKey());
                if (user != null) {
    ...
    @Override
    public JqGridPage<TblUserInfoDTO> getGitLabUserAuth(JqGridPage<TblUserInfoDTO> jqGridPage, Project project) throws URISyntaxException {
        ...
        if (CollectionUtil.isNotEmpty(members)) {
            for (Member member : members) {
                TblUserInfoDTO user = devManageToSystemInterface.getCodeBaseUserDetailByUserScmAccount(member.getUsername());
                if (user != null) {
     ...
     * @param svnPreCommitWebHookParam
     * @return void
     * @Description svn pre-commit钩子验证前准备
    ...
    public void modifySvnPassword(Long currentUserId, String userScmAccount, String userScmPassword, String entryptUserScmPassword) throws Exception {
        //获取用户可以访问的svn工具记录
        List<Map<String, Object>> svnToolList = tblToolInfoMapper.getMySvnToolInfo(currentUserId);
        //方便开发调试用
    ...
    public List<TblSystemScmSubmit> getSystemScmSubmitConfigs(Long systemId, Integer scmType) {
        return tblSystemScmSubmitMapper.getSystemScmSubmits(systemId, scmType);
    }

    @Override
    @Transactional(readOnly = true)
    public List<TblSystemScmSubmitRegex> getSystemScmSubmitRegexConfigs(Long systemId) {
        return tblSystemScmSubmitRegexMapper.getSystemScmSubmitRegexes(systemId);
    }

    @Override
    @Transactional(readOnly = true)
    public List<Project> getSystemGitLabProjects(Long systemId) throws URISyntaxException {
        List<Map<String, Object>> systemGitLabRepositoryLocationInfoList = tblToolInfoMapper.getSystemGitLabRepositoryLocationInfo(systemId);
