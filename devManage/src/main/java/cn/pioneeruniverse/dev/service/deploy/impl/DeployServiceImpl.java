		...
		for (Map<String, Object> sysInfomap : sysInfoList) {
			...
			if (sysInfomap.get("architectureType").toString().equals(Constants.SERVER_MICRO_TYPE)
				...
				for (TblSystemModule tblSystemModule : lists) {
					...
					//} else {// 制品部署
					//moduleParam.put("jobType", 2);
					//}

					// 现在部署信息
//					List<Map<String, Object>> nowModuleMessage = tblSystemJenkinsJobRunMapper
//							.selectModuleBuildMessagesNowAutoDeploy(moduleParam);
					List<Map<String, Object>> nowModuleMessage = tblSystemJenkinsJobRunMapper
							.selectModuleBuildMessagesNowAutoDeployNew(moduleParam);
...
//							.selectModuleBuildMessageAutoDeploy(moduleParam);
					List<Map<String, Object>> moduleMessage = tblSystemJenkinsJobRunMapper
							.selectModuleBuildMessageAutoDeployNew(moduleParam);
