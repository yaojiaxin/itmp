        ...
        colModel:[ 
            ...
            { 
                ...
                formatter : function(value, grid, rows, state) {
                    ...
                    if(deleteModule && toStr(rows.architectureType) == "") {
                    	str += ' | <a class="a_style" style="cursor:pointer" onclick="deleteSystemAndModule('+ rows.id + ',' + rows.systemId + ')">删除</a>';
                    }
                    if(rows.level == 0){
                    	
                    	str += ' | <a class="a_style" style="cursor:pointer" onclick="codeConfig('+ rows.id + ','+ rows.architectureType + ',\''+ rows.systemName +'\')">源码配置</a>';
                    	
                    	if(rows.architectureType == 1 && systemAddModule){
...
//删除子模块
function deleteSystemAndModule(id, systemId){
...
function configAddInfo(data){
	...
	if( data.deployScriptList.length>0 ){
		   for(var i=0;i<data.deployScriptList.length;i++ ){ 
			   ...
			   '<input class="hideIdValue" type="hidden" val='+data.deployScriptList[i].id+' />'+
...
function config( row ){   
	layer.open({
	      ...
	      tipsMore: true,
	      anim: 2,
	      content:'/devManageui/systeminfo/toSystemDeploy?id='+row.id+",createType="+row.createType+",systemId=" +row.systemId+",artifactId=" +row.artifactId+",buildDependency=" +row.buildDependency+",buildDependencySequence=" +row.buildDependencySequence,
	      btn: ['确定','取消'] ,
	      btnAlign: 'c', //按钮居中
	      yes: function(index , layero){
	    	  var body = layer.getChildFrame('body', index );
	...
	return ; 
	//
	/*$("#configInfoForm").data('bootstrapValidator').destroy(); 
    formValidator();
	    	...
	    	data:{
	  		...
	  		success:function(data){ 
                ...
                $("#microServiceNum").val( data.systemInfo.systemCode );
	  			...
	  			$("#architectureId").append('<option '+flag1+' value="1">多模块架构 </option><option '+flag2+' value="2">单模块架构 </option>');
	  			部署方式
	  			var depFlag1 = '';
	  			var depFlag2 = '';
	  			...
	  			$(".microServiceData").append('<div class="singleData"><div class="singleDataDiv"><div class="singleDataFont">环境类型</div></div><div class="singleDataDiv"><div class="singleDataFont">工具名称</div></div><div class="singleDataDiv"><div class="singleDataFont">分支名称</div></div><div class="singleDataDiv"><div class="singleDataFont">源代码地址</div></div></div>');
	  			systemVersionStr = '';
  				commissioningWindow = '';
	  			...
	  			if( data.systemInfo.createType===null ){ 
	  				...
	  				$(".manualInfo").css("display","none");
	  				//$(".microServiceData").append('<div class="singleData"><div class="singcheckBox"><input type="checkbox"  onclick="allCheck()"/></div><div class="singleDataDiv"><div class="singleDataFont">环境类型</div></div><div class="singleDataDiv"><div class="singleDataFont">托管服务</div></div><div class="singleDataDiv"><div class="singleDataFont">工具地址</div></div><div class="singleDataDiv"><div class="singleDataFont">是否提交</div></div><div class="singleDataDiv"><div class="singleDataFont">系统版本</div></div><div class="singleDataDiv"><div class="singleDataFont">投产窗口</div></div><div class="singleDataDiv2"><div class="singleDataFont2">分支名称</div></div><div class="singleDataDiv2"><div class="singleDataFont2">管理地址</div></div><div class="singleDataDiv"><div class="singleDataFont2">提交管理人</div></div><div class="singleDataDiv"><div class="singleDataFont2">提交用户</div></div></div>');
	  				if(data.list.length>0){
	  					sType = data.list[0].scmType;
		  				for(var i=0;i<data.list.length;i++){ 
		  	  				(function (i){  
		  	  					$(".microServiceData").append(
		  	  							'<div class="singleData"><input type="hidden" name="systemScmNeme" value=""/><div class="singleDataDiv"><div class="search_input entrySelect"> <select class="selectpicker  microServiceEnvironmentType" name="environmentTypeName"> <option value="">请选择</option> '+environmentTypeStr+' </select> </div> </div>'+
  	  							'<div class="singleDataDiv"> <div class="search_input entrySelect"> <select class="selectpicker microServiceScmType abc"  name="toolId"> <option value="">请选择</option></select> </div> </div>'+   
  	  							'<div class="singleDataDiv"> <div class="search_input entrySelect2"> <input type="text" class="form-control microServiceScmBranch" placeholder="请输入" /> </div> </div> '+
  	  							'<div class="singleDataDiv"> <div class="search_input entrySelect2"> <input type="text" class="form-control microServiceScmUrl" placeholder="请输入" /> </div> </div>'+
		  	  					...
		  	  					$("select[name='environmentTypeName']").eq(i).attr('typeValue', data.list[i].environmentType);
		  		  				...
		  		  				$("input[name=microServiceScmSubmitSuperUser]").eq(i).val( data.list[i].submitSuperUserNames);
		  		  				$("input[name=microServiceSubmitUserName]").eq(i).val( data.list[i].submitUserNames);
		  		  				$(".microServiceScmSubmitSuperUser").eq(i).val( data.list[i].submitSuperUserName);
		  		  				$(".microServiceSubmitUserName").eq(i).val( data.list[i].submitUserName);
		  				...
		  				$(".microServiceData").append('<div class="singleData"><input type="hidden" name="systemScmNeme" value=""/><div class="singleDataDiv"><div class="search_input entrySelect"> <select class="selectpicker  microServiceEnvironmentType" name="environmentTypeName"> <option value="">请选择</option> <option value="1">DEV</option> <option value="2">SIT</option> <option value="3">UAT</option> <option value="4">PRD</option> </select> </div> </div>'+
  	  							...
  	  							'<div class="singleDataDiv2"> <div class="search_input entrySelect2"> <input type="text" class="form-control microServiceScmBranch" placeholder="请输入" /> </div> </div> <div class="singleDataDiv2"> <div class="search_input entrySelect2"> <input type="text" class="form-control microServiceScmUrl" placeholder="请输入" /> </div> </div>'+
  	  							'<div class="singleDataDiv2"> <div class="search_input entrySelect2"> <input type="hidden" name="microServiceScmSubmitSuperUser" /><input type="text" class="form-control microServiceScmSubmitSuperUser" placeholder="请输入"  onclick="userModalShow(this)"/> </div> </div><div class="singleDataDiv2"> <div class="search_input entrySelect2"><input type="hidden" name="microServiceSubmitUserName" /> <input type="text" class="form-control microServiceSubmitUserName" placeholder="请输入" onclick="userModalShow(this)" /> </div> </div>'+		
		  						'<div class="delSingData"  onclick="delSingData(this)"> <a class="delFont">删除</a> </div> </div> '); 
		  				} 
	  				if(sType != '' ){
	  					getToolId2(sType)
	  				}
	  				...
	  				for( var i=0;i<data.jenkinslist.length;i++ ){
	  					...
	  					var str='<div class="JenkinsJob rowdiv"><input type="hidden" class="hideKeyId" value='+data.jenkinslist[i].rootPom+' /><div class="form-group def_col_10">'+
	  		 			'<div class="JobName font_right fontWeihgt def_col_16">Jenkins工具地址：</div>'+
	  					...
	  					'</select></div></div><div class="form-group def_col_10"><div class="JobName font_right fontWeihgt def_col_16">Jenkins Job Name：</div>'+
	  		 			'<div class="JobName def_col_16"><input disabled="disabled" type="text" value='+data.jenkinslist[i].jobName+' class="form-control" placeholder="请输入" name="JobName"/></div></div>'+
	  		 			'<div class="form-group def_col_10"><div class="JobName font_right fontWeihgt def_col_16">Jenkins Job Path：</div>'+
	  		 			'<div class="JobPath def_col_16"><input disabled="disabled" type="text" value="'+toStr(data.jenkinslist[i].jobPath)+'" class="form-control" placeholder="请输入" name="JobPath"/></div></div>'+
	    	...
	    	data:{
	  		...
	  		success:function(data){   
	  			...
	  			if( data.systemInfo.buildType == '1' || data.systemInfo.buildType === null ){
	  				$("#normalArtifactID").val( row.artifactId );
	  			...
	  			var buildflag1 = '';
	  			var buildflag2 = '';
	  			if(row.buildDependency == 1){
	  				buildflag1 = "selected";
	  			}else if(row.buildDependency == 2){
	  				buildflag2 = "selected";
	  			}
	  			$("#normalBuildDependency").append('<option '+buildflag1+' value="1">必须 </option><option '+buildflag2+' value="2">非必须 </option>');	  			
	  			$("#normalBuildDependencySequence").val( row.buildDependencySequence ); 
	  			$('.selectpicker').selectpicker('refresh');
...
function addSystem(){
    ...
    $.ajax({
        url:"/devManage/systeminfo/addSystem",
        dataType: 'json',
        type:'post',
        data:{
