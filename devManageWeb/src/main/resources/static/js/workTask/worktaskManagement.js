...
function pageInit(){
    ...
    $("#devlist").jqGrid({
        ...
        colNames:['id','任务编号', '任务名称','状态','涉及系统','关联需求','关联开发任务','优先级','任务来源','开发人员','预估剩余工作量(人天)','操作'],
        colModel:[
            ...
            {name:'featureCode',index:'featureCode',
                formatter : function(value, grid, rows) {
                    value == null?value='':'';
                    return "<a class='a_style' onclick='showDevTask("+rows.requirementFeatureId+")'>"+value+"</a>";
                }
            },
            {name:'devTaskPriority',index:'devTaskPriority',width:75},
            {name:'requirementFeatureSource',index:'requirementFeatureSource',width:100,
                formatter : function(value, grid, rows) {
            ...
            {name:'estimateRemainWorkload',index:'estimateRemainWorkload',width:130},
            {
                name:'操作',
                index:'操作',
                align:"left",
                ...
                formatter : function(value, grid, rows, state) {
                    ...
                    var codeOpt = "<a onclick='restoreSearchOptions()' href='/devManageui/codeReviewControl/toReviewPage?devTaskScmId=" + (rows.tbtsID == null ? "" : rows.tbtsID) + "&devTaskId=" + (rows.devTaskId == null ? "" : rows.devTaskId) + "&scmType=" + (rows.scmType == null ? "" : rows.scmType) + "&codeReviewOrViewResult=2'>代码查看</a>";
                    var opt_status = [];
...
function showDevTask(id) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $("#checkdevTaskStatus").text(data.statusName);
...
function PostAssig() {
	...
	obj.userName =$("#assignUser").val();
	 obj.devUserId=$("#assignUser").attr("assignUserID");
...
function exportPerson_btn(){
    ...
    window.location.href="/devManage/worktask/getExcelAllWork?excelDate="+obj+"&getProjectIds="+getProjectIds;
}
