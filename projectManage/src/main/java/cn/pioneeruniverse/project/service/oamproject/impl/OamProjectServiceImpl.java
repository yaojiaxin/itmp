...
//			 根据用户名查询出用户id
		...
		if(StringUtils.isEmpty(projectManageName) && StringUtils.isNotEmpty(developManageName)) {
			Integer code = getUserPost("项目开发岗");
//			 根据用户名查询出用户id
		...
		if(StringUtils.isNotEmpty(projectManageName) && StringUtils.isNotEmpty(developManageName)) {
			...
			Integer code2 = getUserPost("项目开发岗");
			...
			if(userId1 == null || userId2 == null) {
				...
				List<Long> ids1 = projectGroupUserMapper.findProjectGroupIdsByUserIdAndUserPost(map1);
				// 根据项目组主键id查询出项目id
				...
				List<Long> ids2 = projectGroupUserMapper.findProjectGroupIdsByUserIdAndUserPost(map2);
				// 根据项目组主键id查询出项目id
...
//			List<String> systemName = systemInfoMapper.selectSystemName(id);
			...
			for (String key : jsonObj.keySet()) {
				if(jsonObj.get(key).toString().equals("项目开发岗")) {
	...
	public void insertOamProject(TblProjectInfo tblProjectInfo,HttpServletRequest request) throws Exception {
		// TODO Auto-generated method stub
